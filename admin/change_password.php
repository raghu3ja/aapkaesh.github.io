...
if (isset($_POST['submit'])) {
    ...
    $result = mysqli_query($conn,"SELECT * FROM `admin_tb` WHERE `username`='admin' AND `password`='".$_REQUEST['currentPassword']."'");
    if(mysqli_affected_rows($conn)>0){
		if ($_POST["newPassword"] == $_POST["confirmPassword"]) {
        mysqli_query($conn, "UPDATE `admin_tb` SET `password`='".$_POST["newPassword"]."' WHERE `username`='admin'");
