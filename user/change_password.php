...
if (isset($_POST['submit'])) {
    ...
    $result = mysqli_query($conn,"SELECT * FROM `users` WHERE `id`='".$_SESSION['user_id']."' AND `password`='".$_REQUEST['currentPassword']."'");
    if(mysqli_affected_rows($conn)>0){
		if ($_POST["newPassword"] == $_POST["confirmPassword"]) {
        mysqli_query($conn, "UPDATE `users` SET `password`='".$_POST["newPassword"]."' WHERE `id`='" . $_SESSION["user_id"] . "'");
